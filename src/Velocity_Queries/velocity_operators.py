...
class VelocityBigQueryHook(GoogleCloudBaseHook, DbApiHook):
    ...
    def __init__(self,
        ...
        super(VelocityBigQueryHook, self).__init__(
...
class VelocityStreamByDateOperator(BaseOperator):
    """
    ...
    def __init__(self,
        ...
        self.end_date222 = end_date
        self.bigquery_conn_id = bigquery_conn_id
        self.use_legacy_sql = use_legacy_sql
        self.schema_out = schema_out
        super(VelocityStreamByDateOperator, self).__init__(*args, **kwargs)
    ...
    def execute(self, context):
        hook = VelocityBigQueryHook(bigquery_conn_id=self.bigquery_conn_id)
        jobs=[]
        ...
        print self.start_date222
        print self.end_date222
        start = datetime.strptime(self.start_date222,"%Y-%m-%d")
        end = datetime.strptime(self.end_date222,"%Y-%m-%d")
        ...
        for it in range((end-start).days):
            ...
            print "sql="+sql2
            print "destination table = "+dst_table+"$"+partition_short
            while True:
                try:
                    print "Run the job:"
...
class VelocityStreamByDatePartitionListOperator(BaseOperator):
    """
    Incrementally loads data from one table to another, repartitioning if necessary
    """
    ui_color = '#33FFEC'
    ...
    def __init__(self,
        ...
        self.schema_out = schema_out
        super(VelocityStreamByDatePartitionListOperator, self).__init__(*args, **kwargs)
...
class BigQueryPlugin(AirflowPlugin):
    name = "Velocity AirFlow Plugin"
    operators = [
        VelocityStreamByDateOperator,
        VelocityStreamByDatePartitionListOperator
    ]
