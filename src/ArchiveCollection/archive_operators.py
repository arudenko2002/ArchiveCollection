...
class StreamByDateOperator(BaseOperator):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.sql = sql
        self.sql_alternative = sql_alternative
        self.destination_table = destination_table
    ...
    def execute(self, context):
        ...
        for it in range((end-start).days):
            ...
            print "dare="+str(single_date)[:10]
            print "partition="+partition
            print "dest.table="+dst_dataset+"."+dst_table+"$"+partition_short

            sql2=self.sql.replace("{datePartition}",partition).replace("{project}",hook.project_id)
            sql2_alternative=self.sql_alternative.replace("{datePartition}",partition).replace("{project}",hook.project_id)
            counter=0
            while True:
                counter=counter+1
                if counter>10:
                    print "WARNING counter>0 "+partition
                    break
                try:
                    print "Run the job "+partition+":"
                    #print sql2
                    job = hook.write_to_table(sql2,dst_dataset, dst_table+"$"+partition_short,use_legacy_sql=False, write_disposition='WRITE_TRUNCATE')
                    jobs.append(job)
                    print "Run this ("+partition+")"+str(job)+":  "+str(len(jobs))
                    break
                except JobInsertException:
                    print "Problem with the job, waiting..."
                    time.sleep(120)
                    print "try again..."+partition
                    print sql2_alternative
                    sql2=sql2_alternative
                    #job = hook.write_to_table(sql2_alternative,dst_dataset, dst_table+"$"+partition_short,use_legacy_sql=False, write_disposition='WRITE_TRUNCATE')
                    #jobs.append(job)
                    #print "Run this ("+partition+")"+str(job)+":  "+str(len(jobs))
                    #break
                    pass
